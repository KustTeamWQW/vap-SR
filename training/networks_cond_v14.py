...
class SFTLayer(torch.nn.Module):
    ...
    def forward(self, features, conditions):
        ...
        cat_input = cat_input.to(self.mul_conv1.weight.dtype)
...
class UNetBlock_Ref(torch.nn.Module):
    def __init__(self,
        ...
        self.norm0 = GroupNorm(num_channels=in_channels, eps=eps)
        self.sft0 = SFTLayer(nf=in_channels, n_condition=3)
        self.conv0 = Conv2d(in_channels=in_channels, out_channels=out_channels, kernel=3, up=up, down=down,
                            resample_filter=resample_filter, **init)
        self.affine = Linear(in_features=emb_channels, out_features=out_channels * (2 if adaptive_scale else 1), **init)
        self.norm1 = GroupNorm(num_channels=out_channels, eps=eps)
        self.sft1 = SFTLayer(nf=out_channels, n_condition=3)
        self.conv1 = Conv2d(in_channels=out_channels, out_channels=out_channels, kernel=3, **init_zero)
    ...
    def forward(self, x, emb, lr_cond, ref_cond):
        orig = x
        x = self.norm0(x)
        x = self.sft0(x, torch.cat([lr_cond], dim=1))
        x = self.conv0(silu(x))

        x = self.norm1(x)
        x = self.sft1(x, torch.cat([ref_cond], dim=1))
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        self.base_h, self.base_w = img_resolution
        ...
        for level, mult in enumerate(channel_mult):
            res_h = self.base_h >> level
            res_w = self.base_w >> level
            if level == 0:
                cin = cout
                cout = model_channels
                self.enc[f'{res_h}x{res_w}_conv'] = Conv2d(in_channels=cin, out_channels=cout, kernel=3, **init)
            else:
                self.enc[f'{res_h}x{res_w}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True,
                                                              **block_kwargs)
                if encoder_type == 'skip':
                    self.enc[f'{res_h}x{res_w}_aux_down'] = Conv2d(in_channels=caux, out_channels=caux, kernel=0,
                                                                   down=True,
                                                                   resample_filter=resample_filter)
                    self.enc[f'{res_h}x{res_w}_aux_skip'] = Conv2d(in_channels=caux, out_channels=cout, kernel=1,
                                                                   **init)
                if encoder_type == 'residual':
                    self.enc[f'{res_h}x{res_w}_aux_residual'] = Conv2d(in_channels=caux, out_channels=cout, kernel=3,
            ...
            for idx in range(num_blocks):
                ...
                attn = (res_h in attn_resolutions) or (res_w in attn_resolutions)
                self.enc[f'{res_h}x{res_w}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout, attention=attn,
                                                                    **block_kwargs)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                self.dec[f'{res_h}x{res_w}_in0'] = UNetBlock_Ref(in_channels=cout, out_channels=cout, attention=True,
                                                                 **block_kwargs)
                self.dec[f'{res_h}x{res_w}_in1'] = UNetBlock_Ref(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                self.dec[f'{res_h}x{res_w}_up'] = UNetBlock_Ref(in_channels=cout, out_channels=cout, up=True,
                                                                **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = (idx == num_blocks and res_w in attn_resolutions)
                self.dec[f'{res_h}x{res_w}_block{idx}'] = UNetBlock_Ref(in_channels=cin, out_channels=cout,
                                                                        attention=attn,
                                                                        **block_kwargs)
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res_h}x{res_w}_aux_up'] = Conv2d(in_channels=out_channels, out_channels=out_channels,
                                                                 kernel=0, up=True, resample_filter=resample_filter)
                self.dec[f'{res_h}x{res_w}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                self.dec[f'{res_h}x{res_w}_aux_conv'] = Conv2d(in_channels=cout, out_channels=out_channels, kernel=3,
                                                               **init_zero)

    def forward(self, x, noise_labels, lr_cond, ref_cond, class_labels, augment_labels=None):
        # Mapping.
        ...
        # Encoder.
        skips = []
        x = torch.cat([x, lr_cond, ref_cond], dim=1)
        aux = x
        ...
        for name, block in self.dec.items():
            if 'aux_up' in name:
                ...
                x = block(x, emb, lr_cond, ref_cond)
        return aux
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        block_kwargs = dict(emb_channels=emb_channels, channels_per_head=64, dropout=dropout, init=init,
                            init_zero=init_zero, eff_attn=eff_attn)
        ...
        for level, mult in enumerate(channel_mult):
            ...
            for idx in range(num_blocks):
                cin = cout
                cout = model_channels * mult
                self.enc[f'{res_h}x{res_w}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout,
                                                                    attention=((res_h in attn_resolutions) or (
                                                                                res_w in attn_resolutions)),
                                                                    **block_kwargs)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            res_w = self.base_w >> level
            if level == len(channel_mult) - 1:
                self.dec[f'{res_h}x{res_w}_in0'] = UNetBlock(in_channels=cout, out_channels=cout, attention=True,
                                                             **block_kwargs)
                self.dec[f'{res_h}x{res_w}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, **block_kwargs)
            else:
                self.dec[f'{res_h}x{res_w}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True,
                                                            **block_kwargs)
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                self.dec[f'{res_h}x{res_w}_block{idx}'] = UNetBlock(in_channels=cin, out_channels=cout,
                                                                    attention=((res_h in attn_resolutions) or (
                                                                                res_w in attn_resolutions)),
                                                                    **block_kwargs)
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        # drm modify
        self.model = globals()[model_type](img_resolution=img_resolution, in_channels=9, out_channels=img_channels,
                                           label_dim=label_dim, **model_kwargs)

    def forward(self, x, sigma, lr_cond, ref_cond, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        # drm modify
        ...
        c_noise = c_noise.flatten().to(dtype)
        lr_cond = lr_cond.to(dtype)
        ref_cond = ref_cond.to(dtype)

        F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), lr_cond, ref_cond,
                         class_labels=class_labels, **model_kwargs)
        # F_x = self.model((c_in * x).to(dtype), c_noise.flatten(), lr_cond, ref_cond,  class_labels=class_labels)
        F_x = F_x.to(dtype)
        D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x
