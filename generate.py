...
def refdiff_sampler(
    net, latents, ref_images, class_labels=None, randn_like=torch.randn_like,
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        denoised = net(x_hat, t_hat, latents, ref_images, class_labels).to(torch.float64)
        d_cur = (x_hat - denoised) / t_hat
        
        x_next = x_hat + (t_next - t_hat) * d_cur
        
        if i < num_steps - 1:
            denoised = net(x_next, t_next, latents, ref_images, class_labels).to(torch.float64)
    ...
    for i, batch_seeds in tzip(range(len(rank_batches)), rank_batches, unit='batch', disable=(dist.get_rank() != 0)):
      if not os.path.exists(os.path.join(outdir, preload_base_name[i][0])):
        ...
        if sampler_stages == 'first':
            ...
            images = F.interpolate(images, size=(512, 640))
            
        if sampler_stages in ['second', 'both']:
            # Second stage generation.
            images = refdiff_sampler(net_second, images, ref_images, class_labels, randn_like=rnd.randn_like, **second_stage_sampler_kwargs)
